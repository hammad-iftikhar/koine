...
export function ChatPanel({ messages, onSend, onClose }: ChatPanelProps) {
  ...
  return (
    ...
    >
      <header className="flex items-center gap-2.5 border-b border-(--edge) px-4 py-3.5">
        ...
        <button
          ...
          onClick={onClose}
          className="grid h-9.5 w-9.5 place-items-center rounded-full text-fg-2 transition-colors hover:bg-(--pane-2) hover:text-fg focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-blue"
        >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3.5 overflow-auto px-4 py-3.5">
        <p className="m-0 rounded-xl bg-(--well) px-3 py-2.5 text-[12px] text-fg-2">
      ...
      <div className="flex items-center gap-2 border-t border-(--edge) p-3">
        <input
          ...
          className="min-w-0 flex-1 rounded-full border border-(--edge) bg-(--well) px-3.75 py-2.5 text-[14px] text-fg outline-none focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-blue"
        />
        <button
          type="button"
          aria-label="Send"
          onClick={submit}
          className="grid h-9.5 w-9.5 place-items-center rounded-full text-blue transition-colors hover:bg-blue/15 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-blue"
        >
